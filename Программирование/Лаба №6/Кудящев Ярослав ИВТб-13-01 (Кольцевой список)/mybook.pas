...
uses
  ...
  pp,a,b,ii:integer;
  ch:char;
  x,j,i,n,code,y,yy,cec,xx:integer;
s:string;
c:char;
m,k:tlist;
up,dn,au:boolean;
...
case c of
    #97..#122:if i<50 then  begin
        ...
        gotoxy(2,wherey+1);
         ...
         ii:=ii+1;
            ...
            write(c);
            if i=cec+50 then cec:=cec+50;
            end            else begin    s[i]:=c;
    ...
    #65..#90 : if i<50 then begin setlength(s,i); c:=chr(ord(c)+32);
    ...
    #9: tab;
    #8: begin yy:=wherey;xx:=wherex;if ((yy=6) and (xx=2)) then begin gotoxy(2,6); write(' ');gotoxy(2,6); end else if (yy=2) and (xx=2) then begin gotoxy(2,2);write(' ');gotoxy(2,2);end else
    begin  x:=wherex+1;y:=wherey;
    if (x>2) then begin x:=wherex;y:=wherey;delete(s,wherex-1,1);i:=i-1;setlength(s,i);write(' ');gotoxy(x-1,y);end else begin gotoxy(51,y-1);
    x:=wherex;y:=wherey;delete(s,wherex-1,1);i:=i-1;setlength(s,i);write(' ');gotoxy(x-1,wherey);end;
    end;end;
    ...
    #13: menu;
        ...
        c:=readkey;
        case c of
        #72: begin y:=wherey;if (y=2) then gotoxy(wherex,2) else if y=6 then gotoxy(wherex,6) else begin if (com>0) then begin up:=true; point;end;gotoxy(wherex,wherey-1);end;end;
        #75: begin if wherex>2 then begin gotoxy(wherex-1,wherey);end; end;
        #77: begin if wherex<=length(s) then begin gotoxy(wherex+1,wherey);end; end;
        #80: begin if com>0 then begin dn:=true; point;end;gotoxy(wherex,wherey+1);end;
        end;
        end;
until (c=#27);
Free(SpisNach);
