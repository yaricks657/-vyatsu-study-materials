...
var ii,j,kek,iii,kk:integer;
begin
   image2.visible:=false;
   if i=false then begin
   Randomize;
   repeat
   a :=mmm[random(6)+1]; //Генерирует рандомное число
   until a>0;
   end;
   vverh(a);
   b:=b+1;
   i:=true;
   if b=5 then begin //при подъёме на 1 этаж
   i:=false;
   b:=0;
   mm[a]:=a; // Добавляем ведро в массив с ведрами не на 1 этаже
   m[a] := (FindComponent('Shape' + IntToStr(a)) as TShape).Top;
       if m[a]=122 then  //Люлька на 12 этаже?
       begin
       mmm[a]:=0; //  Убираем из массивов номера люлек, если они на 12 этаже
       mm[a]:=0;    //
       for iii:=1 to n do begin // Пуст ли Массив из люлек?
       if mmm[iii]=0 then
       begin
       kek:=kek+1;
       if kek=6 then begin  // Конец, если пуст
       timer1.Enabled:=false;
       ShowMessage('Все люльки достигли 12-го этажа.');
       Button1Click(Sender);
       end;
       end; //else  break; // Если хотя бы 1 люлька не на 12 этаже
       end;
       end else
       for iii:=1 to 6 do // Пуст ли массив с люльками, которые не на 1 этаже?
       begin
       if mm[iii]=0 then
       begin
       kk:=kk+1;
       if kk=6 then exit; //Выходим из процедуры, если пуст
       end else
       begin
       timer2.enabled:=true;
       timer1.enabled:=false;
      // break;// Выходим из цикла, если не пуст
       end;
       end;
   end;
...
image2.Visible:=true;
 Randomize;
   repeat
   aaa :=mm[random(6+1)]; //Генерирует рандомное число
   until aaa>0;
   end;
  //Timer4Timer(Sender);
   //golub(aaa);
   ...
   gg:=false;
   bb:=0;
   m[aaa] := (FindComponent('Shape' + IntToStr(aaa)) as TShape).Top;
    if m[aaa]=672 then
     mm[aaa]:=0;
  timer2.Enabled:=false;
timer1.enabled:=true;
...
begin
  ...
  //pogoda:=1;
  timer3.enabled:=false;
 { r:=Red(ColorToRGB(ColorStart));
g:=Green(ColorToRGB(ColorStart));
h:=Blue(ColorToRGB(ColorStart));
dr := (r - Red(ColorToRGB(ColorEnd))) / 255;//TrackBar1.Max;
dg := (g - Green(ColorToRGB(ColorEnd))) / 255;//TrackBar1.Max;
db := (h - Blue(ColorToRGB(ColorEnd))) / 255;//TrackBar1.Max;}
  ...
  timer4.Enabled:=false;
  Button2.Enabled:=false;
   //Form1.Color := RGBToColor(r - Round(dr*pogoda), g - Round(dg*pogoda), h - Round(db * pogoda));
  nn:=6;
  x:=50;
  image2.visible:=false;
  {for i:=1 to 6 do
  ...
  m[i] := (FindComponent('Shape' + IntToStr(i)) as TShape).Top;
  mmm[i]:= i;
  end;
end;
