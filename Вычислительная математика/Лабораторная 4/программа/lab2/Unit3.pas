...
begin
  ...
  while not stop do
    begin
      ...
      for i:=1 to 4 do
        tmp[i]:=rez[i];
      drow(tmp,x,count); // заполнение таблицы
...
begin
  ...
  if max < 1 then lbl36.Caption:=('Метод сходится норма по строкам||a|| '+ FloatToStr(max)+' < 1')
    else
     begin
      for i:=1 to 4 do
      sum[i]:=0;
      ...
      for i:=2 to 4 do
        if max < sum[i] then max:=sum[i];
      if max < 1 then lbl36.Caption:=('Метод сходится норма по столбцам ||a||'+ FloatToStr(max)+' < 1')
      else lbl36.Caption:=('Метод рассходится  ||a|| '+ FloatToStr(max)+' > 1')
     end;
