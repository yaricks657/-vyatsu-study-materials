...
begin
  ...
  if flagA then form2.strngrd1.Cells[0,0]:='Точка A неподвижна'
  else form2.strngrd1.Cells[0,0]:='Точка B неподвижна';
  count:=1;
  stop:=false;
  while not stop do
    begin
...
begin
  try
    ...
    ShowMessage('Ошибка при считывании данных');
  end;
  combin(x1,x2,e);;
  Form2.Show;
end;
...
begin
    ...
    With PaintBox.Canvas do // оси
      begin
        ...
        TextOut(260, 5, 'Y');
        TextOut(485, 260, 'X');
        TextOut(10, 10, '1 ед. отрезок по X = 0.5');
        TextOut(10, 30, '1 ед. отрезок по Y = 5');
        TextOut(252, 252, '0');
      end;

    PaintBox.Canvas.MoveTo(0, 250);
    for i := 1 to 50 do
      With PaintBox.Canvas do    //штрихи
        begin
           Pen.Color := clBlack;
           Moveto(i*50, y0-3);
           Lineto(i*50, y0+4);
