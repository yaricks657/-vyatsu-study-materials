...
var i,j,k,kk, s: integer; x,y:array [1..100] of real; a:array [1..100,1..100] of real; vr1,vr2,q1,q2,h,yy,p1,p2,sum,xx:real;
begin
 ...
 begin
  ...
  for j:=3 to i do
   begin
    vr1:=vr1*(q1-j+2);
    vr2:=vr2*(q2+j-2);
    Memo1.Lines.Add('*' + FloatToStr(vr1) + ' ' + FloatToStr(vr2));
   end;
  vr1:=vr1/fact(i-1);
  vr2:=vr2/fact(i-1);
  Memo1.Lines.Add('/' + FloatToStr(vr1) + ' ' + FloatToStr(vr2));
  p1:=p1+vr1*a[1,i];
  p2:=p2+vr2*a[k-i+1,i];
  //Memo1.Lines.Add(FloatToStr(p1) + ' ' + FloatToStr(p2) + ' ' + FloatToStr(vr1) + ' ' + FloatToStr(vr2));
 end;
...
StringGrid7.Cells[0,0]:='       Хар';
StringGrid7.Cells[0,1]:='       Хге';
StringGrid7.Cells[0,2]:='       Хга';
...
StringGrid8.Cells[2,0]:='Отклонение';
...
label22.Caption:='Минимальным является: E'+IntToStr(mi);

      case mi of
      1: label21.Caption:='Тип зависимости:'+#13#10+'y=ax+b - линейная';
      2: label21.Caption:='Тип зависимости:'+#13#10+'y=a*(b^x) - показательная';
      3: label21.Caption:='Тип зависимости:'+#13#10+'y=1/(ax+b) - дробно-линейная';
      4: label21.Caption:='Тип зависимости:'+#13#10+'y=a*ln(x)+b - логарифмическая';
      5: label21.Caption:='Тип зависимости:'+#13#10+'y=a*(x^b) - степенная';
      6: label21.Caption:='Тип зависимости:'+#13#10+'y=a+b/x - гиперболическая';
      7: label21.Caption:='Тип зависимости:'+#13#10+'y=x/(ax+b) - дробно-рациональная';
      end;
  ...
  ssq:=0;
   for i:=1 to 10 do ssq:=ssq+sqr({(b*exp(x[i]*ln(a))-y[i])(x[i]/}((a*x[i]+b)-y[i]));
    ssq:=sqrt(ssq/10);
  Label10.Caption:='Ср. квадр. откл.='+FloatToStrF(ssq,ffFixed,8,4);
...
begin
StringGrid1.Cells[0,1]:='0,43';
StringGrid1.Cells[0,2]:='0,48';
StringGrid1.Cells[0,3]:='0,55';
StringGrid1.Cells[0,4]:='0,62';
StringGrid1.Cells[0,5]:='0,70';
StringGrid1.Cells[0,6]:='0,75';
StringGrid1.Cells[1,1]:='1,63597';
StringGrid1.Cells[1,2]:='1,73234';
StringGrid1.Cells[1,3]:='1,87686';
StringGrid1.Cells[1,4]:='2,03345';
StringGrid1.Cells[1,5]:='2,22846';
StringGrid1.Cells[1,6]:='2,83973';

StringGrid3.Cells[0,1]:='1,415';
StringGrid3.Cells[0,2]:='1,420';
StringGrid3.Cells[0,3]:='1,425';
StringGrid3.Cells[0,4]:='1,430';
StringGrid3.Cells[0,5]:='1,435';
StringGrid3.Cells[0,6]:='1,440';
StringGrid3.Cells[0,7]:='1,445';
StringGrid3.Cells[0,8]:='1,450';
StringGrid3.Cells[0,9]:='1,455';
StringGrid3.Cells[0,10]:='1,460';
StringGrid3.Cells[0,11]:='1,465';
StringGrid3.Cells[1,1]:='0,888551';
StringGrid3.Cells[1,2]:='0,889599';
StringGrid3.Cells[1,3]:='0,890637';
StringGrid3.Cells[1,4]:='0,891667';
StringGrid3.Cells[1,5]:='0,892687';
StringGrid3.Cells[1,6]:='0,893698';
StringGrid3.Cells[1,7]:='0,894700';
StringGrid3.Cells[1,8]:='0,895693';
StringGrid3.Cells[1,9]:='0,896677';
StringGrid3.Cells[1,10]:='0,897653';
StringGrid3.Cells[1,11]:='0,898619';
end;
