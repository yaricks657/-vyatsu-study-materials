...
if Abs(FIter1(b))<1 then
 begin
 Btn2.Enabled:=true;
 Form5.Lbl1.Caption:='Процесс сходится';
 Form5.Memo1.Lines.Add('Каноническое уравнение: y=x-(sin(x+0,5)-2*x+1,5)/'+FloatToStr(round(k))+'');
 end
else
 begin
 Btn2.Enabled:=false;
 Form5.Lbl1.Caption:='Процесс расходится';
 Form5.Memo1.Lines.Add('Каноническое уравнение: y=x-(sin(x+0,5)-2*x+1,5)/'+FloatToStr(round(k))+'');
 end;
end;
...
If Form5.Edt2.Text='' then
 begin
 ShowMessage('Введите начальное приближение');
 Exit;
 end
else begin
...
Form5.Memo1.Lines.Add('Решение методом итераций: ');
...
str(x1:7:5,s1);
Form5.Edt1.Text:=s1;
Form5.Memo1.Lines.Add('Число итераций: '+IntToStr(it)+'');
Form5.Memo1.Lines.Add('Ответ: х='+s1+'');
