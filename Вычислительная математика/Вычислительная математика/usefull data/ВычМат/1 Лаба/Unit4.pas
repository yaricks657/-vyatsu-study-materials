...
var s: string;
    fab: real;
begin
 if Form4.Btn1.Caption='Проверить условие M<=2m' then
 begin
 try
 a:=StrToFloat(Edt2.Text);
 b:=StrToFloat(Edt3.Text);
 except
 MessageDlg('Значения введены неверно', mtError, [mbOK], 0);
 Edt2.Text:='';
 Edt3.Text:='';
 Exit;
 end;
 ...
 If Mm>2*m then
  ...
  Form4.lbl2.Caption:='Условие M<=2m не выполняется';
  Form4.Btn1.Caption:='Сузить интервал';
  end
 else
  begin
  Form4.Lbl4.Caption:='M='+FloatToStr(Mm);
  Form4.Lbl5.Caption:='m='+FloatToStr(m);
  Form4.Lbl2.Caption:='Условие M<=2m выполняется';
...
Memo1.Lines.Add('Решение комбинированным методом: ');
...
Cells[0,0]:='№';
Cells[1,0]:='x по недостатку';
Cells[2,0]:='x по избытку';
if ((F1(a)<0) and (F2(a)>0)) or ((F1(a) > 0) and (F2(a) < 0)) then // если а неподв
begin
Memo1.Lines.Add('Неподвижен левый конец a');
Memo1.Lines.Add('X по недостатку находим методом касательных');
Memo1.Lines.Add('X по избытку находим методом хорд');
...
begin
Memo1.Lines.Add('Неподвижен правый конец b');
Memo1.Lines.Add('X по недостатку находим методом хорд');
Memo1.Lines.Add('X по избытку находим методом касательных');
while Abs(xn-xi)>e do
 begin
