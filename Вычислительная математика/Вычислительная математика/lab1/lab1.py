...
def iteration(x, y):
    ...
    if ((f(x) * f(y)) > 0):
        ...
        if (abs(dfi(x0)) < 1):
            ...
            if (dfi(x) < 1) and (dfi(y) < 1):
                q = 1
                print('Тип сходимости: двусторонняя dfi(x) < 1, dfi(y) < 1')
            elif (dfi(x) < 1) and (dfi(y) > 1):    
                q = max(abs(dfi(x)), abs(dfi(y)))
                print('Тип сходимости: односторонняя dfi(x) < 1, dfi(y) > 1')
            elif (dfi(x) > 1) and (dfi(y) < 1):
                q = max(abs(dfi(x)), abs(dfi(y)))
                print('Тип сходимости: односторонняя dfi(x) > 1, dfi(y) < 1')
            else:
