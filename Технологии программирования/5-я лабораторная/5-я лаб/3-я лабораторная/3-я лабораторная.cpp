							...
							case 122:
							{
								...
								if (kyk == true) {
									if (a1 != 0 || a2 != 0) {
										...
										typedef void (*PTrap)(int b, int a, int n, int a4, int arg, double trapp[20], int bbb);
										PTrap Trap = (PTrap)GetProcAddress(hDll, "trap");

										for (int i = 0; i < razmer + 1; i++)
										{
											if (buf[i] == 's' && buf[i + 1] == 'i')
											{
												pryamoygolnik(a1, a2, a3, 1, arg[kek],integ,bb);
												(*Trap)(a1, a2, a3, 1, arg[kek],trapp,bbb);
												kek = kek + 1;
											}
											else
												if (buf[i] == 'c' && buf[i + 1] == 'o')
												{
													pryamoygolnik(a1, a2, a3, 2, arg[kek], integ, bb);
													(*Trap)(a1, a2, a3, 2, arg[kek], trapp, bbb);
													kek = kek + 1;
												}
												else if (buf[i] == 't' && buf[i + 1] == 'g' && buf[i - 1] != 'c')
												{
													pryamoygolnik(a1, a2, a3, 3, arg[kek], integ, bb);
													(*Trap)(a1, a2, a3, 3, arg[kek], trapp, bbb);
													kek = kek + 1;
												}
												else
													if (buf[i] == 'c' && buf[i + 1] == 't')
													{
														pryamoygolnik(a1, a2, a3, 4, arg[kek], integ, bb);
														(*Trap)(a1, a2, a3, 4, arg[kek], trapp, bbb);
														kek = kek + 1;
													}
													else
														if (buf[i] == 'x' && buf[i + 1] != ')' && buf[i + 1] != '^')
														{
															pryamoygolnik(a1, a2, a3, 5, arg[kek], integ, bb);
															(*Trap)(a1, a2, a3, 5, arg[kek], trapp, bbb);
															kek = kek + 1;
														}
														else
															if (buf[i] == 'x' && buf[i + 1] == '^')
															{
																pryamoygolnik(a1, a2, a3, 6, arg[kek], integ, bb);
																(*Trap)(a1, a2, a3, 6, arg[kek], trapp, bbb);
																kek = kek + 1;
															}
															else
																if (buf[i] == 'l' && buf[i + 1] == 'n')
																{
																	pryamoygolnik(a1, a2, a3, 7, arg[kek], integ, bb);
																	(*Trap)(a1, a2, a3, 7, arg[kek], trapp, bbb);
																	kek = kek + 1;
																}
										}
										first = integ[0];
